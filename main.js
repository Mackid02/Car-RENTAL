...
ScrollReveal().reveal(".getstarted__section", {
  distance: "50px",
  duration: 1000,
  origin: "bottom",
  delay: 300,
...
const heading = document.querySelector(".header__container h1");
if (heading) {
  let chars = heading.innerText.split("");
  heading.innerHTML = chars
    .map(
      (char, i) =>
        `<span style="opacity:0;display:inline-block;transform:translateY(20px);transition:all 0.4s ${i * 50}ms ease">${char}</span>`
    )
    .join("");

  window.addEventListener("load", () => {
    document
      .querySelectorAll(".header__container h1 span")
      .forEach((span) => {
